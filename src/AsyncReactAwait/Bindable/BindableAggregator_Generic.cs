...
namespace AsyncReactAwait.Bindable
{
    ...
    /// <typeparam name="TSource">The value type being aggregated.</typeparam>
    public class BindableAggregator<TSource, TRes> : IBindable<TRes>, IBindableRaw
    {
        private readonly IReadOnlyList<IBindable<TSource>> _bindableArr;
        private readonly Func<IReadOnlyList<IBindable<TSource>>, TRes> _aggregator;
        ...
        public TRes Value
        {
            get
            {
                if (!_hasValue)
                {
                    _currentValue = ComputeCurrentValue();
                    _hasValue = true;
                }
        ...
        /// <summary>
        /// Calculates the aggregated value based on the current values of all underlying bindables.
        /// </summary>
        private TRes ComputeCurrentValue()
        {
            return _aggregator.Invoke(_bindableArr);
        }

        /// <summary>
        /// Updates the cached aggregated value.
        /// </summary>
        ...
        public BindableAggregator(IEnumerable<IBindable<TSource>> bindableRaws, Func<IReadOnlyList<IBindable<TSource>>, TRes> aggregator)
        {
        ...
        private void OnSourceUpdated(TSource value)
        {
            ...
            // Keep local variable to avoid repeated field access
        ...
        private void OnSourceUpdatedFull(TSource prevVal, TSource? nextVal)
        {
